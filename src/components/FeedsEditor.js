...
class FeedsEditor extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      openSuccessSaveDialog: false,
      openFailedSaveDialog: false,
    };
    ...
    this.closeSaveDialogHandler = this.closeSaveDialogHandler.bind(this);
  ...
  saveFeedsContentHandler() {
    ...
    let sheetName = this.getFeedSheet(window.report.modifyFeeds);
    let xlsxFileName = this.getFeedSource(window.report.modifyFeeds);
    let optionsContent = this.state.optionsContent;
    for (let i=0; i<optionsContent.length; i++) {
      let option = JSON.stringify(optionsContent[i]);
      optionsArray.push(option);
    }
    let feedsTableContent = typeRow.concat([optionsArray],this.state.feedsContent,sheetName);
    feedsTableContent = JSON.stringify(feedsTableContent);

    let xhr = new XMLHttpRequest();
    xhr.open('POST', `http://localhost:3080/save/${window.report.name}/${xlsxFileName}`, true);
    xhr.setRequestHeader('Content-type', 'application/json');
    xhr.onreadystatechange = function() {
      if(xhr.readyState === 4 && xhr.status === 200) {
          self.setState({
          ...this.state,
          openSuccessSaveDialog: true,
          xhrRequestInProgress: false,
        })
      } else {
        self.setState({
          ...this.state,
          openFailedSaveDialog: true,
          xhrRequestInProgress: false
        })
      }
    };
    xhr.send(feedsTableContent); 
    
    /*fetch('http://localhost:3080/save/', {method: 'POST', headers: {
      'Content-Type': 'application/json;charset=utf-8'
    }, body: '12344', mode: 'no-cors' })
  ...
  render() {
    ...
    if (this.state.xhrRequestInProgress) {
      children.push(
    ...
    if (this.state.contentReady) {
      ...
      if (this.state.openSuccessSaveDialog || this.state.openFailedSaveDialog) {
        dialogChildren.push(
          <Dialog
            open={true}
            aria-labelledby="alert-dialog-title"
          ...
          >
            <DialogContent key="dialogContent">
              <DialogContentText id="alert-dialog-description" key="dialogContentText">
                {this.state.openSuccessSaveDialog ? "Данные успешно сохранены" : "Ошибка сохранения данных, повторите попытку"}
              </DialogContentText>
            </DialogContent>
            <DialogActions key="dialogActions">
              <Button color="primary" key="cancelDeletionButton" onClick={this.closeSaveDialogHandler}>
                OK
              </Button>
            </DialogActions>
          </Dialog>
...
saveFeedsContentHandler() {
    ...
    let feedsTableContent = typeRow.concat([optionsArray],this.state.feedsContent);
    feedsTableContent = JSON.stringify(feedsTableContent);
    let xhr = new XMLHttpRequest();
    xhr.open('POST', 'http://localhost:3080/save/', true);
    xhr.setRequestHeader('Content-type', 'application/json');
    xhr.onreadystatechange = function() {
      if(xhr.readyState === 4 && xhr.status === 200) {
          self.setState({
